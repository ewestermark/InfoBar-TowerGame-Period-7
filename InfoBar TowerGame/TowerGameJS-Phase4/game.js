    ...
    sendEnemies() {
        ...
        for( i = 0; i < numEnemies; i++) {
            for(j = 0; j < 3; j++) { // try 3 times to find valid start cell
                let row = Math.floor(Math.random() * (this.rows/8));    // top  half of rows
                let col = Math.floor(Math.random() * this.cols/8);        // any column
            ...
            if(j < 3) { // if we found a valid cell to start the enemy
                ...
                //this.enemiesCount = this.enemiesCount + numEnemies;
                }
            }
    }
  ...
  updateInfoElements(time){
    ...
    for(let i = 0; i < infoElements.length; i++){
      ...
      if(info.innerHTML.indexOf('Bank') != -1){
        ...
        info.innerHTML = 'Wave <br/>' + 1;
      }else if(info.innerHTML.indexOf('Enemies') != -1){
        info.innerHTML = 'Enemies <br/>' + this.enemiesCount;
      }else if(info.innerHTML.indexOf('Health') != -1){
